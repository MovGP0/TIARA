...
| Component path | TinaAskVoltagesDlg.BtnPanel.GroupBox1.CurrentsCB |
...
| Graph node | `resource:dfm:TinaAskVoltagesDlg/TinaAskVoltagesDlg.BtnPanel.GroupBox1.CurrentsCB` |
...
The recovered form field order maps `CurrentsCB` to the check box at offset `0x718`. A checked state includes rows that start with the third recovered category marker. A clear state excludes those rows. The exact marker text is not recovered, but the DFM caption and field mapping identify this filter as **Currents**. Rows that pass the filter are split into node and value text and copied to the grid. The final helper sets the row count and updates the grid, panel, and form heights. It shows at most 12 row heights before the grid must handle more rows.
...
```mermaid
flowchart TD
    currentsClick["Change Currents"] --> filterHandler["FilterCBClick at 012b8950"]
    filterHandler --> rebuildGrid["Clear both grid columns"]
    rebuildGrid --> readRows["Read each source row and all four check states"]
    readRows --> currentDecision{"Currents is checked and row has marker 3?"}
    currentDecision -->|Yes| addRow["Split node and value and add the row"]
    currentDecision -->|No| skipRow["Do not add it through this category"]
...
- Current graph evidence: The DFM maps four check boxes to `FilterCBClick`. The recovered form field order maps `CurrentsCB` to offset `0x718`. `FUN_012b6470` reads this check state and accepts rows with the third category marker only when it is set.
- Complexity: simple
