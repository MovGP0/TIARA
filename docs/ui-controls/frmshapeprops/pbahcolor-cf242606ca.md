...
> Analysis status: Reviewed against the recovered shared handler, sender branch, paint path, and nearby label.
...
| --- | --- |
| Form | frmShapeProps |
| Component path | frmShapeProps.gbArrowHead.pbAHColor |
...
| Graph node | `resource:dfm:frmShapeProps/frmShapeProps.gbArrowHead.pbAHColor` |
| Handler node | `function:00c5b7e0` |
| Graph layer | UI |

## What happens when clicked

The paint box opens a color dialog with the shared arrow-head and background color. If the user accepts the dialog, the handler stores the selected color. If the user cancels it, the stored color stays unchanged. The handler then repaints both the arrow-head and background previews. The nearby recovered label states that an X in the arrow-head preview means that the border color is used.

## Click flow

```mermaid
flowchart TD
    control["Click the arrow-head color preview"] --> dialog["Open the color dialog with the shared color"]
...
- Rank 1: (X = same as border color) at distance 64.
- Rank 2: &Head color: at distance 103.
- Rank 3: &Start head type: at distance 146.
