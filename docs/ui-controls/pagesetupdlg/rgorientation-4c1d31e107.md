...
`rgOrientation` sits on PageSetupDlg - the page setup dialog: paper size, width and height, orientation, and the margins around the drawing.

The resource binds its `OnClick` to `rgOrientationClick`. That binding is all the image holds: `rgOrientationClick` has no address, no body and no call edges, so what the click does beyond reaching the handler is not recovered.

## Click flow

```mermaid
flowchart TD
    control["Orientation (TRadioGroup)"] -->|"OnClick"| handler["rgOrientationClick"]
    handler -.->|"no address, no body, no edges"| gone["unknown"]
```

## Inputs

- Whatever `rgOrientationClick` reads from the form. Not known: the handler is not in the image.
...
- Not not known. Nothing in the image says what `rgOrientationClick` writes.
...
- Not known. Whether `rgOrientationClick` can fail, and what it does when there is nothing to do, is not in the image.
...
- The other controls on this form that do something: `PageSetupDlg` (Page Setup), `OKBtn` (bkOK), `cbPaperSize` (TComboBox), `eTop` (TEdit), `eBottom` (TEdit), `eLeft` (TEdit), `eRight` (TEdit), `eWidth` (TEdit).
...
- Recovering `rgOrientationClick` needs the class table, which needs the protected pages. Watching the running program would settle what the control does without settling how, and for this form not attempted, the form not being reachable from the Schematic Editor menus.
