...
`OKBtn` sits on NewMacroCompForm - the older New Macro Wizard: a name, a content file, a shape file, default label and parameters, and three ways to say where the macro comes from.
...
What `OKBtnClick` did in this older form is not in the image, and there is no longer a reason to want it: the behaviour the build actually runs belongs to `TfMacroWiz`, which is recovered in full.

## Click flow

```mermaid
flowchart TD
    control["OK (TBitBtn)"] -->|"OnClick"| handler["OKBtnClick"]
...
- Not not known. Nothing in the image says what `OKBtnClick` writes.
...
- The other controls on this form that do something: `NewMacroCompForm` (New Macro Wizard), `sbBrowseShape` (TSpeedButton), `sbBrowseContent` (TSpeedButton), `EName` (TEdit), `chkCurrCircuit` (&Current circuit), `chkAutoShape` (&Auto-generated), `chkEmptyCircuit` (&Empty circuit).
...
- Recovering `OKBtnClick` needs the class table, which needs the protected pages. Watching the running program would settle what the control does without settling how, and for this form the command that would have opened it was watched, and it opened `TfMacroWiz` instead.
