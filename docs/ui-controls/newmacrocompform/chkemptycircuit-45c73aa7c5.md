...
| Component path | NewMacroCompForm.chkEmptyCircuit |
| Control class | TCheckBox |
| Caption | &Empty circuit |
| Hint | Not present in the recovered resource. |
| Kind | Not present in the recovered resource. |
| Handler name | chkEmptyCircuitClick |
| Handler address | Not in the image - see below. |
| Graph node | `resource:dfm:NewMacroCompForm/NewMacroCompForm.chkEmptyCircuit` |
| Handler node | `concept:dfm-handler:TNewMacroCompForm/chkEmptyCircuitClick` |
| Graph layer | UI |

## What happens when clicked

`chkEmptyCircuit` sits on NewMacroCompForm - the older New Macro Wizard: a name, a content file, a shape file, default label and parameters, and three ways to say where the macro comes from.
...
What `chkEmptyCircuitClick` did in this older form is not in the image, and there is no longer a reason to want it: the behaviour the build actually runs belongs to `TfMacroWiz`, which is recovered in full.

## Click flow

```mermaid
flowchart TD
    control["Empty circuit (TCheckBox)"] -->|"OnClick"| handler["chkEmptyCircuitClick"]
...
- Whatever `chkEmptyCircuitClick` reads from the form. Not known: the handler is not in the image.
...
- Not not known. Nothing in the image says what `chkEmptyCircuitClick` writes.
...
- Not known. Whether `chkEmptyCircuitClick` can fail, and what it does when there is nothing to do, is not in the image.
...
- The other controls on this form that do something: `NewMacroCompForm` (New Macro Wizard), `sbBrowseShape` (TSpeedButton), `sbBrowseContent` (TSpeedButton), `OKBtn` (OK), `EName` (TEdit), `chkCurrCircuit` (&Current circuit), `chkAutoShape` (&Auto-generated).
...
- Recovering `chkEmptyCircuitClick` needs the class table, which needs the protected pages. Watching the running program would settle what the control does without settling how, and for this form the command that would have opened it was watched, and it opened `TfMacroWiz` instead.
