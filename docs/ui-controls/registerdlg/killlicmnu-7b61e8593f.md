...
| --- | --- |
| Form | RegisterDlg |
| Component path | RegisterDlg.MainMenu1.KillLicMnu |
| Control class | TMenuItem |
| Caption | Kill license |
| Hint | Not present in the recovered resource. |
| Kind | Not present in the recovered resource. |
| Handler name | KillLicMnuClick |
| Handler address | Not in the image - see below. |
| Graph node | `resource:dfm:RegisterDlg/RegisterDlg.MainMenu1.KillLicMnu` |
| Handler node | `concept:dfm-handler:TRegisterDlg/KillLicMnuClick` |
| Graph layer | UI |

## What happens when clicked

`KillLicMnu` sits on RegisterDlg - the licence dialog, captioned `Authorize`. It carries the registration fields, a menu for moving a licence between machines, and the contact details for doing it by hand.

The resource binds its `OnClick` to `KillLicMnuClick`. That binding is all the image holds: `KillLicMnuClick` has no address, no body and no call edges, so what the click does beyond reaching the handler is not recovered.

## Click flow

```mermaid
flowchart TD
    control["Kill license (TMenuItem)"] -->|"OnClick"| handler["KillLicMnuClick"]
    handler -.->|"no address, no body, no edges"| gone["unknown"]
```

## Inputs

- Whatever `KillLicMnuClick` reads from the form. Not known: the handler is not in the image.
...
- Not not known. Nothing in the image says what `KillLicMnuClick` writes.
...
- Not known. Whether `KillLicMnuClick` can fail, and what it does when there is nothing to do, is not in the image.
...
- Recovering `KillLicMnuClick` needs the class table, which needs the protected pages. Watching the running program would settle what the control does without settling how, and for this form not attempted: it belongs to licensing, and pressing its controls on a real installation could move or destroy a licence.
