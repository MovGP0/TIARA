...
| --- | --- |
| Form | frmShapeProps |
| Component path | frmShapeProps.gbArrowHead.sbAHColor |
...
| Graph node | `resource:dfm:frmShapeProps/frmShapeProps.gbArrowHead.sbAHColor` |
| Handler node | `function:00c5b7e0` |
| Graph layer | UI |

## What happens when clicked

The ellipsis button opens a color dialog with the shared arrow-head and background color. If the user accepts the dialog, the handler stores the selected color. If the user cancels it, the stored color stays unchanged. The handler then repaints both the arrow-head and background previews. The nearby recovered label states that an X in the arrow-head preview means that the border color is used. The extracted ellipsis glyph agrees with the dialog action.

## Click flow

```mermaid
flowchart TD
    control["Click the arrow-head ellipsis button"] --> dialog["Open the color dialog with the shared color"]
    dialog --> accepted{"Did the user accept a color?"}
    accepted -- "Yes" --> store["Store the shared head and fill color"]
    accepted -- "No" --> keep["Keep the shared color"]
    store --> repaint["Repaint the arrow-head and fill previews"]
    keep --> repaint
```
...
- Extracted glyph: [`0192_frmShapeProps_frmShapeProps_gbArrowHead_sbAHColor_Glyph_Data.png`](../../../glyph/0192_frmShapeProps_frmShapeProps_gbArrowHead_sbAHColor_Glyph_Data.png)

## Nearby label candidates

Nearby labels are layout candidates only. They are not proof of behavior.

- Rank 1: (X = same as border color) at distance 142.
- Rank 2: &Head color: at distance 181.
- Rank 3: &Start head type: at distance 224.
