/* Ghidra address: 00798fd0 */
/* Ghidra symbol: FUN_00798fd0 */


undefined8 FUN_00798fd0(longlong param_1,undefined8 param_2)

{
  undefined1 local_28 [32];
  
  FUN_00417580(local_28,&DAT_0075cb10);
  FUN_00417c40(local_28,*(longlong *)(*(longlong *)(param_1 + 8) + 8) +
                        (longlong)*(int *)(param_1 + 0x10) * 0x20,&DAT_0075cb10);
  FUN_00417c40(param_2,local_28,&DAT_0075cb10);
  FUN_00417740(local_28,&DAT_0075cb10);
  return param_2;
}

