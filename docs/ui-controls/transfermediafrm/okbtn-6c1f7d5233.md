...
`OKBtn` sits on TransferMediaFrm - the dialog that moves a licence onto transfer media.
...
- The other controls on this form that do something: `TransferMediaFrm` (Transfer license), `CancelBtn` (bkCancel).
...
- Recovering `OKBtnClick` needs the class table, which needs the protected pages. Watching the running program would settle what the control does without settling how, and for this form not attempted: it belongs to licensing, and pressing its controls on a real installation could move or destroy a licence.
