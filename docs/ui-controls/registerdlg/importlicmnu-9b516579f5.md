...
`ImportLicMnu` sits on RegisterDlg - the licence dialog, captioned `Authorize`. It carries the registration fields, a menu for moving a licence between machines, and the contact details for doing it by hand.

The resource binds its `OnClick` to `ImportLicMnuClick`. That binding is all the image holds: `ImportLicMnuClick` has no address, no body and no call edges, so what the click does beyond reaching the handler is not recovered.

## Click flow

```mermaid
flowchart TD
    control["3. Import license (TMenuItem)"] -->|"OnClick"| handler["ImportLicMnuClick"]
    handler -.->|"no address, no body, no edges"| gone["unknown"]
```

## Inputs

- Whatever `ImportLicMnuClick` reads from the form. Not known: the handler is not in the image.
...
- Not not known. Nothing in the image says what `ImportLicMnuClick` writes.
...
- Not known. Whether `ImportLicMnuClick` can fail, and what it does when there is nothing to do, is not in the image.
...
- Recovering `ImportLicMnuClick` needs the class table, which needs the protected pages. Watching the running program would settle what the control does without settling how, and for this form not attempted: it belongs to licensing, and pressing its controls on a real installation could move or destroy a licence.
