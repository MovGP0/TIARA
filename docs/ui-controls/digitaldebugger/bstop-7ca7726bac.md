...
`bStop` sits on DigitalDebugger - the digital debugger, which steps a digital circuit one state at a time.

The resource binds its `OnClick` to `bStopClick`. That binding is all the image holds: `bStopClick` has no address, no body and no call edges, so what the click does beyond reaching the handler is not recovered.

## Click flow

```mermaid
flowchart TD
    control["Stop (TButton)"] -->|"OnClick"| handler["bStopClick"]
    handler -.->|"no address, no body, no edges"| gone["unknown"]
```

## Inputs

- Whatever `bStopClick` reads from the form. Not known: the handler is not in the image.
...
- Not not known. Nothing in the image says what `bStopClick` writes.
...
- The other controls on this form that do something: `DigitalDebugger` (Digital Debugger), `bNext` (Next >>).
...
- Recovering `bStopClick` needs the class table, which needs the protected pages. Watching the running program would settle what the control does without settling how, and for this form not attempted, the form not being reachable from the Schematic Editor menus.
