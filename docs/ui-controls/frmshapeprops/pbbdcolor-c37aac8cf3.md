...
| --- | --- |
| Form | frmShapeProps |
| Component path | frmShapeProps.gbBackground.pbBdColor |
...
| Graph node | `resource:dfm:frmShapeProps/frmShapeProps.gbBackground.pbBdColor` |
| Handler node | `function:00c5b7e0` |
| Graph layer | UI |

## What happens when clicked

The paint box opens a color dialog with the shared background and arrow-head color. If the user accepts the dialog, the handler stores the selected color. If the user cancels it, the stored color stays unchanged. The handler then repaints both the background and arrow-head previews. The recovered code does not keep separate values for these two previews.

## Click flow

```mermaid
flowchart TD
    control["Click the fill color preview"] --> dialog["Open the color dialog with the shared color"]
...
- Rank 1: &Fill color: at distance 103.
