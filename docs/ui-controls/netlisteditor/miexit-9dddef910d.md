...
The handler does not inspect `Sender`, so `MIExit` and the toolbar close button use the same path. Any modified-document prompt belongs to the form's close-query event, not this wrapper.

## Click flow

```mermaid
flowchart TD
    control["Click Close Netlist Editor"] --> handler["FUN_01532450"]
