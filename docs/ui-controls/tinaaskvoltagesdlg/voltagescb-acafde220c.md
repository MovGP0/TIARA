...
The recovered form field order maps `VoltagesCB` to the check box at offset `0x708`. A checked state includes rows that start with the first recovered category marker. A clear state excludes those rows. The exact marker text is not recovered, but the DFM caption and field mapping identify this filter as **Nodal Voltages**. Rows that pass the filter are split into node and value text and copied to the grid. The final helper sets the row count and updates the grid, panel, and form heights. It shows at most 12 row heights before the grid must handle more rows.
...
```mermaid
flowchart TD
    voltagesClick["Change Nodal Voltages"] --> filterHandler["FilterCBClick at 012b8950"]
    filterHandler --> rebuildGrid["Clear both grid columns"]
    rebuildGrid --> readRows["Read each source row and all four check states"]
    readRows --> voltageDecision{"Nodal Voltages is checked and row has marker 1?"}
    voltageDecision -->|Yes| addRow["Split node and value and add the row"]
    voltageDecision -->|No| skipRow["Do not add it through this category"]
...
- Current graph evidence: The DFM maps four check boxes to `FilterCBClick`. The recovered form field order maps `VoltagesCB` to offset `0x708`. `FUN_012b6470` reads this check state and accepts rows with the first category marker only when it is set.
- Complexity: simple
