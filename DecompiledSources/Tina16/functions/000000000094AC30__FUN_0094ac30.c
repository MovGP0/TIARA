/* Ghidra address: 0094ac30 */
/* Ghidra symbol: FUN_0094ac30 */


int FUN_0094ac30(longlong param_1)

{
  char cVar1;
  int iVar2;
  longlong *plVar3;
  longlong *plVar4;
  int iVar5;
  int local_3c;
  undefined8 local_38;
  undefined8 local_30 [2];
  
  local_38 = 0;
  local_30[0] = 0;
  local_3c = 0;
  plVar4 = *(longlong **)(param_1 + 0x18);
  if (plVar4 == (longlong *)0x0) goto LAB_0094ad81;
  cVar1 = (**(code **)(*plVar4 + 0x168))(plVar4);
  iVar5 = 0;
  if (cVar1 == '\x01') {
    (**(code **)(*plVar4 + 0xe8))(plVar4,local_30);
    iVar2 = FUN_00416420(local_30[0],*(undefined8 *)(param_1 + 0x10));
    iVar5 = 0;
    if (iVar2 != 0) {
      iVar2 = FUN_00416420(*(undefined8 *)(param_1 + 0x10),&LAB_0094adc0);
      if (iVar2 != 0) goto LAB_0094acbf;
    }
    local_3c = 1;
  }
LAB_0094acbf:
  do {
    cVar1 = (**(code **)(*plVar4 + 0x278))(plVar4);
    if (cVar1 == '\0') {
      plVar3 = (longlong *)(**(code **)(*plVar4 + 0x158))(plVar4);
    }
    else {
      plVar3 = (longlong *)(**(code **)(*plVar4 + 0x128))(plVar4);
      iVar5 = iVar5 + 1;
    }
    while (plVar3 == (longlong *)0x0) {
      iVar5 = iVar5 + -1;
      if (iVar5 < 1) break;
      plVar4 = (longlong *)(**(code **)(*plVar4 + 0x170))(plVar4);
      plVar3 = (longlong *)(**(code **)(*plVar4 + 0x158))(plVar4);
    }
    if (iVar5 < 1) break;
    cVar1 = (**(code **)(*plVar3 + 0x168))(plVar3);
    if (cVar1 == '\x01') {
      (**(code **)(*plVar3 + 0xe8))(plVar3,&local_38);
      iVar2 = FUN_00416420(local_38,*(undefined8 *)(param_1 + 0x10));
      if (iVar2 != 0) {
        iVar2 = FUN_00416420(*(undefined8 *)(param_1 + 0x10),&LAB_0094adc0);
        if (iVar2 != 0) goto LAB_0094ad76;
      }
      local_3c = local_3c + 1;
    }
LAB_0094ad76:
    plVar4 = plVar3;
  } while (0 < iVar5);
LAB_0094ad81:
  FUN_004145c0(&local_38,2);
  return local_3c;
}

