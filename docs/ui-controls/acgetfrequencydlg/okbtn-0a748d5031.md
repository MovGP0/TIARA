...
A read-only scan of the rebuilt runtime found exactly one Pascal short string for `TACGetFrequencyDlg`. Its length byte is at file offset `0x33D2D7C`, which maps to virtual address `0x037D8F7C`. The bytes before this string end with `TPF0`, so this instance is the DFM class name. The complete image has no 64-bit pointer to the length byte or to the first character of the name. Therefore, this string cannot identify a VMT class-name slot.

The `OKBtnClick` name for this form starts at file offset `0x33D2FF0`, which maps to virtual address `0x037D91F0`. Its preceding bytes contain the DFM `OnClick` property encoding. The image also contains published-method records named `OKBtnClick` for other classes. Without a VMT or published-method-table boundary for `TACGetFrequencyDlg`, those common-name records cannot be assigned to this form. This missing class boundary prevents an address mapping for the handler and a field-offset map for the form class.
