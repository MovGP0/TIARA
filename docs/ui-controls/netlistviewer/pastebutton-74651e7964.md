...
| --- | --- |
| Form | NetlistViewer |
| Component path | NetlistViewer.BtnPanel.PasteButton |
...
| Graph node | `resource:dfm:NetlistViewer/NetlistViewer.BtnPanel.PasteButton` |
| Handler node | `function:014b58c0` |
| Graph layer | UI |

## What happens when clicked

The toolbar button calls the same handler as the **Paste** menu item. If the click is delivered in the form's window context, it pastes compatible standard clipboard text into the writable `Memo`. SynEdit preserves normal, line, or column selection mode and records one grouped undo action. A read-only editor or clipboard without supported text is a no-op. If another window owns the command context, the handler forwards a paste command to that window instead of changing `Memo`.

## Click flow

```mermaid
flowchart TD
    control["Click Paste toolbar button"] --> handler["FUN_014b58c0"]
    handler --> context{"Form owns the command context?"}
    context -->|No| forward["Forward paste command to active window"]
...
- Extracted glyph: [`0287_NetlistViewer_NetlistViewer_BtnPanel_PasteButton_Glyph_Data.png`](../../../glyph/0287_NetlistViewer_NetlistViewer_BtnPanel_PasteButton_Glyph_Data.png)
...
- The recovered forwarding message is numeric command `0x302`; the rebuilt source does not give it a symbolic name.
