...
`EmailLB` sits on RegisterDlg - the licence dialog, captioned `Authorize`. It carries the registration fields, a menu for moving a licence between machines, and the contact details for doing it by hand.

The resource binds its `OnClick` to `EmailLBClick`. That binding is all the image holds: `EmailLBClick` has no address, no body and no call edges, so what the click does beyond reaching the handler is not recovered.

## Click flow

```mermaid
flowchart TD
    control["register@designsoftware.com (TLabel)"] -->|"OnClick"| handler["EmailLBClick"]
    handler -.->|"no address, no body, no edges"| gone["unknown"]
```

## Inputs

- Whatever `EmailLBClick` reads from the form. Not known: the handler is not in the image.
...
- Not not known. Nothing in the image says what `EmailLBClick` writes.
...
- Not known. Whether `EmailLBClick` can fail, and what it does when there is nothing to do, is not in the image.
...
- The other controls on this form that do something: `RegisterDlg` (Authorize), `OKBtn` (OK), `CancelBtn` (bkCancel), `PageCtrl` (TPageControl), `OrderNumEB` (TEdit), `WebLB` (http://www.designsoftware.com), `OrderNoEB` (TEdit), `InitTrMediaMnu` (1. Initialize transfer media).
...
- Recovering `EmailLBClick` needs the class table, which needs the protected pages. Watching the running program would settle what the control does without settling how, and for this form not attempted: it belongs to licensing, and pressing its controls on a real installation could move or destroy a licence.
