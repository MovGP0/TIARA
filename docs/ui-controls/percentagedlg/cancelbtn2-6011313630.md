# Cancel from the preview-controls page
...
| Component path | PercentageDlg.BtnNotebook.tsCancelPreview.CancelBtn2 |
| Parent page | tsCancelPreview |
| Control class | TBitBtn |
| Button kind | bkCancel |
| Handler name | CancelBtnClick |
| Handler address | 01af11d0 |
| Graph node | `resource:dfm:PercentageDlg/PercentageDlg.BtnNotebook.tsCancelPreview.CancelBtn2` |
...
The handler does not inspect `Sender`. `CancelBtn2` and `CancelBtn1` use the same code. This control is on the `tsCancelPreview` page beside Preview and Pause or Run.

## Click flow

```mermaid
flowchart TD
    click["Click CancelBtn2"] --> flag["Set cancel and close-permission flag"]
...
- `CancelBtn2` has `Kind = bkCancel` and shares `BtnNotebook.tsCancelPreview` with Preview and Pause or Run.
