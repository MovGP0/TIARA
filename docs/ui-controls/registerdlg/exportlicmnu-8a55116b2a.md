...
| --- | --- |
| Form | RegisterDlg |
| Component path | RegisterDlg.MainMenu1.TransferLicMnu.ExportLicMnu |
| Control class | TMenuItem |
| Caption | 2. Export license |
| Hint | Not present in the recovered resource. |
| Kind | Not present in the recovered resource. |
| Handler name | ExportLicMnuClick |
| Handler address | Not in the image - see below. |
| Graph node | `resource:dfm:RegisterDlg/RegisterDlg.MainMenu1.TransferLicMnu.ExportLicMnu` |
| Handler node | `concept:dfm-handler:TRegisterDlg/ExportLicMnuClick` |
| Graph layer | UI |

## What happens when clicked

`ExportLicMnu` sits on RegisterDlg - the licence dialog, captioned `Authorize`. It carries the registration fields, a menu for moving a licence between machines, and the contact details for doing it by hand.

The resource binds its `OnClick` to `ExportLicMnuClick`. That binding is all the image holds: `ExportLicMnuClick` has no address, no body and no call edges, so what the click does beyond reaching the handler is not recovered.

## Click flow

```mermaid
flowchart TD
    control["2. Export license (TMenuItem)"] -->|"OnClick"| handler["ExportLicMnuClick"]
    handler -.->|"no address, no body, no edges"| gone["unknown"]
```

## Inputs

- Whatever `ExportLicMnuClick` reads from the form. Not known: the handler is not in the image.
...
- Not not known. Nothing in the image says what `ExportLicMnuClick` writes.
...
- Not known. Whether `ExportLicMnuClick` can fail, and what it does when there is nothing to do, is not in the image.
...
- Recovering `ExportLicMnuClick` needs the class table, which needs the protected pages. Watching the running program would settle what the control does without settling how, and for this form not attempted: it belongs to licensing, and pressing its controls on a real installation could move or destroy a licence.
